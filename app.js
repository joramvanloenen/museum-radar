const state={page:'overview',opps:[],sources:[],meta:{},history:{},settings:{},selected:null,modal:null};
...
async function load(){migrateLocalData();const [opps,sources,meta,history]=await Promise.all([fetch('./data/opportunities.json').then(r=>r.json()),fetch('./data/sources.json').then(r=>r.json()),fetch('./data/meta.json').then(r=>r.json()),fetch('./data/historical-intelligence.json').then(r=>r.json()).catch(()=>({}))]);state.opps=opps;state.sources=sources;state.meta=meta;state.history=history;state.settings=store.get('radarSettings',{company_name:'Museum Radar',capability_profile:'Interactive museum experiences, physical-digital installations, projection, realtime 3D, sensors, playful learning.',weight_fit:25,weight_budget:10,weight_geo:10,weight_timing:15,weight_confidence:15,weight_strategy:25});render()}
function nav(){
...
function activity(){
  const h=state.history||{},fc=h.fit_counts||{};
  const buyers=(h.top_buyers||[]).slice(0,6);
  const types=(h.project_types||[]).filter(x=>(x.high||0)+(x.adjacent||0)>0).slice(0,6);
  return `${header('Intelligence','Live changes plus historical patterns that improve opportunity scoring.')}<div class="grid4"><div class="metric"><b>${h.generated_from_records||0}</b><span>Historical records</span></div><div class="metric"><b>${fc.high||0}</b><span>Strong-fit examples</span></div><div class="metric"><b>${fc.adjacent||0}</b><span>Adjacent examples</span></div><div class="metric"><b>${(h.learned_terms||[]).length}</b><span>Learned terms</span></div></div><section class="section"><div class="sectionhead"><h2>Recurring useful buyers</h2></div><div class="cards">${buyers.map(b=>`<div class="card source"><h3>${escapeHtml(b.buyer)}</h3><p><b>${b.high||0}</b> strong-fit · ${b.adjacent||0} adjacent · ${b.total||0} total records</p></div>`).join('')}</div></section><section class="section"><div class="sectionhead"><h2>What the history says to look for</h2></div><div class="cards">${types.map(t=>`<div class="card source"><h3>${escapeHtml(t.name)}</h3><p>${t.high||0} strong-fit · ${t.adjacent||0} adjacent examples</p></div>`).join('')}</div></section><section class="section"><div class="sectionhead"><h2>Recent live intelligence</h2></div><div class="card">${mergedOpps().slice().sort((a,b)=>(b.updated_at||'').localeCompare(a.updated_at||'')).map(o=>`<div class="source" data-open="${escapeHtml(o.id)}" style="border-bottom:1px solid var(--line);cursor:pointer"><div class="row space"><b>${escapeHtml(o.title)}</b><span class="muted">${escapeHtml((o.updated_at||'').slice(0,10))}</span></div><div class="muted">${escapeHtml(o.stage)} · score ${o.score} · ${(o.evidence||[]).length} sources</div></div>`).join('')}</div></section>`;
}
