...
def ted(limit=100):
    ...
    for r in rows:
        ...
        out.append({'id':'ted-'+(pub or fp(title,org)), 'title':title,'organization':org,'country':'','city':'','stage':'Live Tender','score':score,'confidence':86,'currency':'EUR','deadline':str(val('deadline',''))[:10] or None,'procurement_window':None,'summary':'Relevant EU procurement notice detected by the automated TED scout.','fit_rationale':'Matched museum / exhibition / interactive procurement language.','pitch_angle':'Review the source notice before deciding whether to bid.','next_action':'Open the TED notice and verify scope, eligibility, budget and deadline.','sample':False,'source_key':'ted','external_id':pub,'updated_at':now(),'evidence':[{'date':str(val('publication-date',''))[:10],'kind':'Procurement','title':'TED notice detected','detail':'Automated discovery. Verify the complete notice at the source.','source_url':f'https://ted.europa.eu/en/notice/-/detail/{pub}' if pub else '','strength':86}]})
    return out
...
def rss(url,name='',country='',limit=100):
    ...
    for title,desc,url,pub in rows:
        ...
        if score<25: continue
        out.append({'id':'rss-'+fp(url or title,pub),'title':title,'organization':name or 'Web signal','country':country,'city':'','stage':stage_from(title+' '+clean),'score':min(score,78),'confidence':58,'currency':'EUR','deadline':None,'procurement_window':'Unknown — investigate','summary':clean[:900],'fit_rationale':'Early web signal matching museum / exhibition / interactive project language.','pitch_angle':'Investigate before contacting; this may be upstream of formal procurement.','next_action':'Follow the source, identify project owner, funding, design team and expected opening date.','sample':False,'source_key':'rss','external_id':url or fp(title,pub),'updated_at':now(),'evidence':[{'date':pub[:10],'kind':'Web signal','title':'Relevant project signal detected','detail':clean[:500],'source_url':url,'strength':58}]})
    return out
