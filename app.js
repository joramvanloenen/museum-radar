...
function overview(){const s=stats(),o=mergedOpps().filter(x=>!x.rejected);const urgent=o.filter(x=>x.score>=80||x.stage==='Live Tender').slice(0,5);const future=o.filter(x=>['Signal','Lead','Expected','Pre-market'].includes(x.stage)).slice(0,4);return `${header('Museum Radar','')}<div class="desktopNotice notice">Shared discoveries refresh automatically. Personal Watch/Reject state stays on this device.</div>
...
function activity(){return `${header('Intelligence Activity','Recent evidence and opportunity changes.')}<div class="card">${mergedOpps().slice().sort((a,b)=>(b.updated_at||'').localeCompare(a.updated_at||'')).map(o=>`<div class="source" data-open="${escapeHtml(o.id)}" style="border-bottom:1px solid var(--line);cursor:pointer"><div class="row space"><b>${escapeHtml(o.title)}</b><span class="muted">${escapeHtml((o.updated_at||'').slice(0,10))}</span></div><div class="muted">${escapeHtml(o.stage)} · score ${o.score} · ${(o.evidence||[]).length} sources</div></div>`).join('')}</div>`}
...
else if(state.modal==='detail'){const o=state.selected;wrap.innerHTML=`<div class="modal"><div class="row space"><span class="badge ${escapeHtml(o.stage)}">${escapeHtml(o.stage)}</span><button class="btn" data-close aria-label="Close">×</button></div><div class="detailgrid"><div class="detail"><div class="kicker">${escapeHtml(o.organization)} · ${escapeHtml(o.country||'')}</div><h1>${escapeHtml(o.title)}</h1><p class="sub">${escapeHtml(o.summary||'')}</p><h3>Why it fits</h3><p>${escapeHtml(o.fit_rationale||'No fit rationale yet.')}</p><h3>Pitch / approach</h3><p>${escapeHtml(o.pitch_angle||'No pitch angle yet.')}</p><h3>Found here / documents</h3>${resourceSection(o)}<h3>Sources</h3>${(o.evidence||[]).map(e=>`<div class="evidence"><div class="evidenceItem"><b>${escapeHtml(e.title)}</b><small>${escapeHtml(e.date||'')} · ${escapeHtml(e.kind||'Sources')} · strength ${e.strength||0}%</small><div>${escapeHtml(e.detail||'')}</div>${safeUrl(e.source_url)?`<a class="muted evidenceSource" href="${escapeHtml(safeUrl(e.source_url))}" target="_blank" rel="noopener noreferrer">Open source ↗</a>`:''}</div></div>`).join('')||'<div class="notice">No sources recorded yet.</div>'}</div><div class="detail"><div class="bigscore ${sc(o.score)}">${o.score}</div><div class="muted">Opportunity score</div><h3>Confidence</h3><div>${o.confidence||0}%</div><h3>Estimated value</h3><div>${fmtMoney(o)}</div><h3>Timing</h3><div>${escapeHtml(o.deadline||o.procurement_window||'Unknown')}</div><h3>Next action</h3><div>${escapeHtml(o.next_action||'—')}</div><div style="height:18px"></div><button class="btn" id="watch">${o.watched?'★ Watched':'☆ Watch'}</button> <button class="btn" id="reject">${o.rejected?'Restore':'Reject'}</button></div></div></div>`}
