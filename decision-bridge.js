...
(function(){
  ...
  window.overview=function(){
    ...
    return `${header('Museum Radar','What should we act on?',true,false)}
      ...
      <section class="section homeActions"><div class="sectionhead"><div><div class="sectionKicker">PRIORITY</div><h2>What should we do?</h2><p>Best current actions — not simply the highest-scoring museum projects.</p></div><button class="textBtn" data-nav="opportunities">All opportunities</button></div><div class="actionList">${act.map(actionCard).join('')||'<div class="notice">No immediate actions. Radar is watching the pipeline.</div>'}</div></section>
