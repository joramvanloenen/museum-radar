...
// Separates project attractiveness from the practical route for a specialist interactive studio.
(function(){
  ...
  function decision(o){
    ...
    if(route==='SUBCONTRACT') action='Contact the lead bidder/main contractor and position the studio as the specialist interactive partner.';
    ...
    if(route==='DIRECT BID') action='Review eligibility and tender requirements now; this appears structured for a specialist interactive supplier.';
  ...
  window.overview=function(){
    ...
    return `${header('Museum Radar','What should we act on?',true,false)}
      ...
      <div class="actionMetrics">
        <button data-nav="opportunities"><b>${direct.length}</b><span>Direct bids</span><small>Likely suitable for a direct bid</small></button>
      ...
      <section class="section homeActions"><div class="priorityBar"><div class="sectionKicker">PRIORITY</div><button class="textBtn" data-nav="opportunities">All opportunities</button></div><div class="actionList">${act.map(actionCard).join('')||'<div class="notice">No immediate actions. Radar is watching the pipeline.</div>'}</div></section>
      ...
      <section class="section"><div class="sectionhead"><div><div class="sectionKicker">EARLY</div><h2>Worth watching</h2><p>Strong projects where the right commercial route is not ready yet.</p></div><button class="textBtn" data-nav="future">Future Radar</button></div><div class="compactCards">${future.map(actionCard).join('')||'<div class="notice">No strong early-stage leads right now.</div>'}</div></section>
  ...
  if(typeof baseModal==='function') window.renderModal=function(){baseModal();if(state.modal!=='detail'||!state.selected)return;const modal=document.querySelector('.detailModal');if(!modal)return;const o=getOpp(state.selected.id),d=o?.decision||decision(o||state.selected),main=modal.querySelector('.detailMain'),facts=modal.querySelector('.detailFacts');if(main){const box=document.createElement('section');box.className='decisionBox';box.innerHTML=`<div class="decisionHead"><span class="routeBadge ${routeClass(d.route)}">${escapeHtml(d.route)}</span><strong>${d.overall} opportunity</strong></div><div class="decisionGrid"><div><b>${d.capability}</b><span>Package fit</span></div><div><b>${d.access}</b><span>Access</span></div><div><b>${d.commercial}</b><span>Commercial</span></div><div><b>${d.strategic}</b><span>Strategic</span></div></div><p><b>Recommended action:</b> ${escapeHtml(d.action)}</p>`;main.querySelector('h1')?.insertAdjacentElement('afterend',box)}if(facts){const label=facts.querySelector('.scoreLabel');if(label)label.textContent='Opportunity score'}};

  const css=document.createElement('style');
  css.textContent=`.homeIntro{display:flex;align-items:center;justify-content:space-between;gap:16px;margin:4px 0 18px;padding:12px 0;color:var(--muted,#9ca7b5)}.homeIntro>div{font-weight:650;color:var(--text,#eef3f8)}.homeLiveDot{display:inline-block;width:7px;height:7px;border-radius:50%;background:#65d49b;margin-right:8px}.actionMetrics{display:grid;grid-template-columns:repeat(4,1fr);gap:10px;margin-bottom:28px}.actionMetrics button{text-align:left;border:1px solid var(--line,#283241);background:rgba(255,255,255,.025);border-radius:12px;padding:16px;color:inherit;cursor:pointer}.actionMetrics button:hover{background:rgba(255,255,255,.05)}.actionMetrics b{display:block;font-size:28px;line-height:1;margin-bottom:8px}.actionMetrics span{display:block;font-weight:700}.actionMetrics small{display:block;margin-top:4px;opacity:.55}.sectionKicker{font-size:10px;font-weight:800;letter-spacing:.13em;opacity:.48;margin-bottom:4px}.priorityBar{display:flex;align-items:center;justify-content:space-between;margin-bottom:10px}.priorityBar .sectionKicker{margin:0}.homeActions .sectionhead p,.sectionhead p{margin:5px 0 0;color:var(--muted,#9ca7b5);font-size:13px}.actionList{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px}.actionCard{border:1px solid var(--line,#283241);border-radius:14px;padding:17px;background:rgba(255,255,255,.025);cursor:pointer;min-width:0}.actionCard:hover{background:rgba(255,255,255,.05);transform:translateY(-1px)}.actionTop{display:flex;align-items:center;justify-content:space-between;gap:8px}.actionRoute,.routeBadge{display:inline-flex;font-size:10px;font-weight:850;letter-spacing:.07em;padding:6px 8px;border-radius:6px;background:rgba(255,255,255,.1)}.actionRoute.direct-bid,.routeBadge.direct-bid{background:rgba(77,205,137,.16);color:#8be4b5}.actionRoute.pitch-now,.routeBadge.pitch-now{background:rgba(116,151,255,.16);color:#a9bdff}.actionRoute.subcontract,.actionRoute.find-main-contractor,.actionRoute.contact-winner,.routeBadge.subcontract,.routeBadge.find-main-contractor,.routeBadge.contact-winner{background:rgba(244,181,76,.14);color:#f5c979}.actionScore{font-size:18px;font-weight:800}.actionCard h3{font-size:19px;line-height:1.22;margin:12px 0 5px}.actionOrg{font-size:12px;color:var(--muted,#9ca7b5)}.actionWhy{display:flex;gap:12px;flex-wrap:wrap;margin:13px 0 0;font-size:11px;color:var(--muted,#9ca7b5)}.actionWhy b{color:var(--text,#eef3f8)}.actionCard p{font-size:13px;line-height:1.45;margin:11px 0;color:var(--text,#eef3f8)}.actionOpen{padding-top:10px;border-top:1px solid var(--line,#283241);font-size:11px;font-weight:700;opacity:.6;display:flex;justify-content:space-between}.compactCards{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:10px}.compactCards .actionCard p{display:none}.compactCards .actionWhy{margin-bottom:12px}.updateStrip{display:grid;grid-template-columns:repeat(3,1fr);gap:8px}.updateStrip button{text-align:left;padding:13px;border:1px solid var(--line,#283241);border-radius:10px;background:transparent;color:inherit;cursor:pointer}.updateStrip b,.updateStrip span{display:block}.updateStrip b{font-size:14px;line-height:1.25;margin-bottom:4px}.updateStrip span{font-size:10px;opacity:.55}.decisionBox{margin:18px 0;padding:18px;border:1px solid var(--line,#283241);border-radius:14px;background:rgba(255,255,255,.025)}.decisionHead{display:flex;align-items:center;justify-content:space-between;gap:12px;margin-bottom:14px}.decisionGrid{display:grid;grid-template-columns:repeat(4,1fr);gap:8px}.decisionGrid>div{padding:10px;border-radius:9px;background:rgba(255,255,255,.04)}.decisionGrid b{display:block;font-size:20px}.decisionGrid span{font-size:11px;opacity:.65}.decisionBox p{margin:14px 0 0;line-height:1.45}@media(max-width:900px){.actionMetrics{grid-template-columns:repeat(2,1fr)}.actionList{grid-template-columns:1fr}.compactCards{grid-template-columns:1fr}.updateStrip{grid-template-columns:1fr}.homeIntro{align-items:flex-start;flex-direction:column}.homeIntro small{display:none}}@media(max-width:520px){.main{padding-left:12px!important;padding-right:12px!important}.topbar{margin-bottom:8px!important}.topbar .title{font-size:24px!important}.topbar .sub{font-size:12px!important}.homeIntro{display:none}.actionMetrics{grid-template-columns:repeat(4,minmax(0,1fr));gap:5px;margin-bottom:16px}.actionMetrics button{padding:9px 7px;border-radius:9px;text-align:center}.actionMetrics b{font-size:20px;margin-bottom:4px}.actionMetrics span{font-size:10px;line-height:1.1}.actionMetrics small{display:none}.section{margin-top:18px!important}.sectionhead{margin-bottom:8px!important}.sectionhead h2{font-size:18px!important}.sectionhead p{display:none}.sectionKicker{font-size:9px;margin-bottom:2px}.priorityBar{margin-bottom:7px}.textBtn{font-size:11px!important}.actionList,.compactCards{gap:6px}.actionCard{padding:10px 11px;border-radius:10px}.actionTop{gap:6px}.actionRoute,.routeBadge{font-size:8px;padding:4px 6px;border-radius:5px}.actionScore{font-size:16px}.actionCard h3{font-size:17px;line-height:1.18;margin:7px 0 3px}.actionOrg{font-size:10px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.actionWhy{gap:8px;margin-top:7px;font-size:9px}.actionCard p{font-size:11px;line-height:1.3;margin:7px 0;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden}.actionOpen{padding-top:6px;font-size:9px}.compactCards .actionWhy{margin-bottom:7px}.updateStrip{gap:5px}.updateStrip button{padding:9px 10px;border-radius:8px}.updateStrip b{font-size:13px;line-height:1.2;margin-bottom:2px}.updateStrip span{font-size:9px}.decisionBox{margin:10px 0;padding:12px}.decisionHead{margin-bottom:9px}.decisionGrid{grid-template-columns:repeat(4,1fr);gap:5px}.decisionGrid>div{padding:7px 5px;text-align:center}.decisionGrid b{font-size:16px}.decisionGrid span{font-size:8px}.decisionBox p{font-size:11px;line-height:1.35;margin-top:9px}}
`;
